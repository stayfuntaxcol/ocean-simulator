...
function matrices(life){return MICRO_TYPES.map(type=>{const mesh=life.root.getObjectByName(type);return Array.from(mesh.instanceMatrix.array.slice(0,mesh.count*16));});}
...
test('all five creatures and tiny fish have finite coloured anatomy with cheaper detail levels',()=>{
  for(const type of [...MICRO_TYPES,'minnow']){
    ...
    for(const name of ['position','normal','color','microFlex'])assert.ok(a.attributes[name].array.every(Number.isFinite));
    assert.ok(a.attributes.position.count/3<5500);assert.ok(b.attributes.position.count<a.attributes.position.count);
    ...
    if(type==='shell')assert.ok(size.x>size.y*4);
...
test('bounded instanced population follows terrain and excludes occupied ground',()=>{
  ...
  assert.equal(life.root.children.length,7,'Five species, one shoal mesh and one particle layer');
  assert.ok(life.stats.bottom>0);
  for(const type of MICRO_TYPES){const mesh=life.root.getObjectByName(type);assert.ok(mesh.isInstancedMesh&&mesh.count<=24);for(const p of positions(mesh)){assert.ok(Math.abs(p.y-terrain(p.x,p.z)-.016)<1e-5);assert.ok(!obstacle.clone().expandByScalar(.65).containsPoint(p));}}
...
test('world anchored positions are deterministic, pause exactly and survive toggles',()=>{
  ...
  assert.notDeepEqual(matrices(a.life)[4],before[4],'Crab makes a short movement');
...
test('terrain edits and removed habitats rebuild safely without saved creature records',()=>{
  ...
  for(const type of MICRO_TYPES)for(const p of positions(life.root.getObjectByName(type)))assert.ok(Math.abs(p.y+15.984)<1e-5);
...
test('distant schools and layered plankton stay submerged and respect independent switches',()=>{
  ...
  const fish=life.root.getObjectByName('minnow');assert.ok(fish.count>0);
  for(let frame=0;frame<50;frame++){
    life.update(.08,camera,{quality:'high'});
    for(const p of positions(fish)){assert.ok(p.y<19.25&&p.y>-17.3);assert.ok(p.distanceTo(camera.position)>=11);}
  }
  // Centers retain safe separation as each group turns; no packed intersecting fish.
  const all=positions(fish);for(let i=0;i<all.length;i++)for(let j=i+1;j<all.length;j++)assert.ok(all[i].distanceTo(all[j])>.75);
...
test('quality decreases geometry and populations, and all allocated resources dispose once',()=>{
  ...
  life.update(0,camera,{quality:'low'});assert.ok(mesh.geometry.attributes.position.count<high);assert.ok(life.stats.bottom<=MICRO_QUALITY.low.patches*5);assert.ok(life.stats.fish<=48);assert.ok(life.stats.plankton<=160);
  life.update(0,camera,{quality:'invalid'});assert.ok(life.stats.bottom<=70);
...
test('micro shaders preserve lighting and fog; review imports actual models and app handles pause',()=>{
  ...
  const url=new URL('../graphics/micro-life-review.html',import.meta.url),html=fs.readFileSync(url,'utf8'),script=html.match(/<script type="module">([\s\S]*?)<\/script>/)[1];
